                    ...
                    if pkgs is not None:
                        for pkg in pkgs:
                            ...
                            for vers in filteredVersions:
                                ...
                                env.AppName = versId.packageId()
                                env.AppVersion = versId.versionId()
                                features = versId.features()
                                if len(features) > 0:
                                    env.Extension['Keywords'] = list(features)
                                
                                if vers.description() is not None:
                                    env.Description = vers.description()
                                if vers.referenceURL() is not None:
                                    env.Repository = vers.referenceURL()
